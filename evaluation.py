...
def evaluate_multi(all_pred, all_labels):
    ...
    print(
        "\nAccuracy:",
        sklearn.metrics.accuracy_score(y_true=all_labels, y_pred=all_pred),
    )
    print(
        "Precision:",
        sklearn.metrics.precision_score(
...
#     ENCODE_DIM = 128
#     LABELS = 86
#     BATCH_SIZE = 50  # Adjust batch size to reduce memory consumption
#     USE_GPU = True
#     MAX_TOKENS = word2vec.vectors.shape[0]
#     EMBEDDING_DIM = word2vec.vectors.shape[1]
#     device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
...
def evaluation():
    ...
    total_loss = 0.0
    total = 0.0
    correct_non_exact = 0  # Counter for correct predictions including non-exact matches
    i = 0
    model.load_state_dict(
        torch.load(
            "./saved_model/best_" + args.input + ".pt", map_location=torch.device("cpu")
    ...
    print("Testing results(Acc):", total_acc / total)
    print(
        "Correct predictions (including non-exact matches):", correct_non_exact / total
    )
    evaluate_multi(all_preds, all_labels)
